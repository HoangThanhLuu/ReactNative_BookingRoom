...
const PlaceScreen = ({ route, navigation }) => {
  ...
  //dùng để cho hiển thi ra khi người dùng bấm vào tìm hiếm thì nó hiển thị ra (sắp xếp,Lọc,Bản đồ)
  const screensPlaces = data?.filter((item) => item.place === route.params.place)
  console.log(route);
  useLayoutEffect(() => {
    ...
    <View style={{flex:1}}>
      ...
      >
        <Pressable style={{ flexDirection: "row", alignItems: "center" }}>
         <Octicons name="arrow-switch" size={24} color={"#00BDD5"} />
          <Text
            style={{
              fontSize: 15,
              color: "#00BDD5",
        ...
        <Pressable style={{ flexDirection: "row", alignItems: "center" }}>
          <Ionicons name="filter" size={24} color="#00BDD5" />
          <Text
            style={{
              fontSize: 15,
              color: "#00BDD5",
        ...
        <Pressable onPress={() => navigation.navigate("MapScreen",{
          searchResults:screensPlaces
        })} style={{ flexDirection: "row", alignItems: "center" }}>
          <FontAwesome5 name="map-marker-alt" size={22} color="#00BDD5" />
          <Text
            style={{
              fontSize: 15,
              color: "#00BDD5",
      ...
      <ScrollView style={{ backgroundColor: "#F5F5F5" }}>
        {/* Khi người dùng bấm vào tìm hiếm => dữ liệu của người dùng chọn nó sẽ đỗ qua đây và nếu bằng với dữ liệu data ở trên thì hiển thị các thông tin phòng cho người dùng   */}
        {data.filter((item) => item.place === route.params.place).map((item) =>  item.properties.map((property, index) => (
          <PropertyCard 
          key={index}
